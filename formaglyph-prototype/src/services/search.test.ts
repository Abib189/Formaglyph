...
describe("catalog search", () => {
  ...
  it("finds semantic tags and applies deterministic filters", () => {
    const matches = searchIcons(iconResults, "payment successful", { category: "Payments", weight: "regular" });
    expect(matches.length).toBeGreaterThan(0);
    expect(matches.every((icon) => icon.category === "Payments" && icon.weight === "regular")).toBe(true);
  });
