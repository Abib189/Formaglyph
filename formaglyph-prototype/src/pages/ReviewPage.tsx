...
function ReviewIconCell({ label, weight, after = false }: { label: string; weight: IconWeight; after?: boolean }) {
...
export function ReviewPage() {
  const { state, addComment, toggleComment, requestChanges, approveProposal } = useAppState();
  ...
  return (
    <main className="page-shell review-page">
      ...
      <div className={approved ? "decision-bar approved" : changesRequested ? "decision-bar changes-requested" : "decision-bar"}>
        ...
        <div><button className="primary-action" onClick={approveProposal} disabled={state.proposal.status !== "in_review"}>{approved ? <Check size={19} /> : <Package size={19} />}{approved ? `Approved for v${state.proposal.targetVersion}` : `Approve for v${state.proposal.targetVersion}`}</button><button className="secondary-action" onClick={requestChanges} disabled={state.proposal.status !== "in_review"}>Request changes</button></div>
      </div>
    </main>
  );
}
